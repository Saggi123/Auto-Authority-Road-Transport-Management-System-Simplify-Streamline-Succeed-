...
app.get('/view_fines', async(req, res)=>{
    if(req.session.loggedin){
      try{
        pool.query('SELECT i.doi,i.payableby, uv.reg_plate, f.offence, f.fine_amount, DATE_FORMAT(i.doi, "%a %b %e %Y") AS formatted_doi,DATE_FORMAT(i.payableby, "%a %b %e %Y") AS formatted_payableby FROM fines f JOIN impose_fine i ON f.fine_id = i.fine_id JOIN user_vehicle uv ON uv.reg_plate = i.veh_reg_no JOIN user_register ur ON ur.user_id = uv.user_id WHERE ur.user_id = ?', [req.session.user], function(err, result){
          if(err) throw err;
        ...
        console.log(duefine);
          ...
          for(let i = 0; i < result.length; i++) {
            ...
            total_fine += data[i].fine_amount
          }
          res.render('view_fines', { fine: data, total_fine: total_fine, payableby:arr })
        })      
      }
      catch(error){
        res.status(500).send(error.message)
      ...
      var date=new Date();
        ...
        pool.query('SELECT * from user_vehicle WHERE reg_plate = ?', [vehicle], (err, result)=>{
          if(err)
            ...
            if(fine >=1 && fine <= 93){
              pool.query('INSERT INTO impose_fine (veh_reg_no, fine_id, doi,payableby) VALUES (?,?,?,?)', [vehicle, fine, date,doi], (err)=>{
                if(err)
...
app.get('/fastag',(req,res)=>{
  res.render('fastag');
 });

 var regno;
 app.post('/checkfastag',async(req,res)=>{
  regno=req.body.vehicleno;
  const query='Select reg_plate from user_vehicle where reg_plate=?';
  pool.query(query,[regno],async(err,result)=>{
    if(err)
    console.log(err);
    else
  {
    if(result.length==0)
    res.render('fastag',{msg:true});
    else if(result.length==1)
    {
      const query1='Select user_id from user_vehicle where reg_plate=?';
      pool.query(query1,[regno],async(err,result1)=>{
        ...
        {
          if(result1[0].user_id!=req.session.user)
          res.render('fastag',{msg1:true});
          else if(result1[0].user_id=req.session.user)
          {
            const query2='Select fast_tag from user_vehicle  where reg_plate=?'
            pool.query(query2,[regno],async(err,result2)=>{
              console.log(result2)
              if(result2[0].fast_tag==null)
              {
                var fastag = regno.match(/\d+([A-Z]+\d+)?/);
         
              res.render('fastag1',{fastag_id:fastag[0]});

              }
              else if(result2[0].fast_tag!=null)
              {
                console.log(result2[0].fast_tag)
                const query3='Select balance from fast_tag where fast_tag_id=?'
                pool.query(query3,[result2[0].fast_tag],async(err,result3)=>{
                  res.render('fastag2',{balance:result3[0].balance,fastag_id:result2[0].fast_tag})
                })
              }
            })
          }
 ...
 app.post('/updatefastag',async(req,res)=>{
  const fast_tag=req.body.fastagid;
  var balance=parseInt(req.body.balance);
  var topup=parseInt(req.body.topupbalance);
  balance=parseInt(balance+topup);
  const Query='Update fast_tag Set balance=? where fast_tag_id=?';
  pool.query(Query,[balance,fast_tag],(err,result)=>{
    if(err)
    console.log(err)
  })
  res.redirect('/dashboard');
 })
 var fast_tagid,balance;
 app.post('/enterfastag',async(req,res)=>{
fast_tagid=req.body.fastagid;
balance=req.body.balance;
const query='Insert into fast_tag (fast_tag_id,balance) values (?,?)';
pool.query(query,[fast_tagid,balance],async(err,result)=>{
  if(err)
  console.log(err);
const query1='Update user_vehicle set fast_tag=? where reg_plate=?';
pool.query(query1,[fast_tagid,regno],async(err,result1)=>{
 if(err)
 console.log(err)
})
})
